...
root.configure(bg="#FCF8E8") 

title=tk.Label(root,text="Mood Tracker⭐", font=("Helvetica", 18, "bold"),bg="#FCF8E8",fg="#333")
title.pack(pady=10)
...
ask_user_label=tk.Label(root,text=random_ask_user, font=("Times New Roman",15),bg="#FCF8E8", fg="#333", wraplength=600) #wraplength=control text wrapping,will break the text into new line once it reaches specific pixel width.
...
button_happy=tk.Button(frame_button,text="Happy😊", font=("Arial",10), command=lambda:set_mood("Happy")) #command=lambda is to bind a function to button/expression 
button_sad=tk.Button(frame_button,text="Sad😢", font=("Arial",10), command=lambda:set_mood("Sad"))       #when button clicked lambda calls set_mood("") function
button_angry=tk.Button(frame_button,text="Angry😠", font=("Arial",10), command=lambda:set_mood("Angry"))
button_excited=tk.Button(frame_button,text="Excited😆", font=("Arial",10), command=lambda:set_mood("Excited"))
button_sleepy=tk.Button(frame_button,text="Sleepy😴", font=("Arial",10), command=lambda:set_mood("Sleepy"))
button_relaxed=tk.Button(frame_button,text="Relaxed😌", font=("Arial",10), command=lambda:set_mood("Relaxed"))

...
class Timer:
    def __init__(self):
        ...
        #Title
        title=tk.Label(self.root,text="Breathing Exercise",font=("Segoe UI",18,"bold"), bg="#DDEFFB", fg="#2B69D5")
        title.pack(pady=30)
        
        backbutton_frame = ctk.CTkFrame(self.root, width=30, height=30, corner_radius=50, fg_color="#DDEFFB")
        backbutton_frame.pack(side='top', anchor='w', padx=20, pady=20)

        back_button = ctk.CTkButton(backbutton_frame, text="⬅", width=30)
    ...
    def animate_resize(self, start_x0, end_x0, start_x1, end_x1, duration):
        ...
        def animate():
            ...
            if progress < 1.0 and self.is_running: #Animation not complete yet
                 self.animation_job = self.root.after(int(1000 / fps), animate)
